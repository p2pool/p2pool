...
combine_bins = math2.add_dicts_ext(lambda a, b: (a[0] + a[1], b[0] + b[1]), (0,0))
...
class DataStreamDescription(object):
    def __init__(self, dataview_descriptions, is_gauge=True, multivalues=False, multivalues_keep=20, multivalues_squash_key=None, multivalue_undefined_means_0=False, default_func=None):
        ...
        self.keep_largest_func = keep_largest(multivalues_keep, multivalues_squash_key, key=lambda t_c: t_c[0]/t_c[1] if self.is_gauge else t_c[0], add_func=lambda a, b: (a[0] + a[1], b[0] + b[1]))
