...
def get_web_root(wb, datadir_path, bitcoind_warning_var, stop_event=variable.Event()):
    ...
    def update_stat_log():
        ...
        stat_log.append(dict(
            ...
            current_payout=node.get_current_txouts().get(bitcoin_data.pubkey_hash_to_script2(bitcoin_data.hash160(wb.my_pubkey)), 0)*1e-6,
            peers=dict(
                incoming=sum(1 for peer in node.p2p_node.peers.itervalues() if peer.incoming),
                outgoing=sum(1 for peer in node.p2p_node.peers.itervalues() if not peer.incoming),
            ),
    ...
    def add_point():
        ...
        hd.datastreams['current_payout'].add_datum(t, current_txouts.get(bitcoin_data.pubkey_hash_to_script2(bitcoin_data.hash160(wb.my_pubkey)), 0)*1e-6)
        miner_hash_rates, miner_dead_hash_rates = get_local_rates()
