...
class CachingWorkerBridge(object):
    ...
    def get_work(self, *args):
        ...
        # Convert merged_addresses dict to hashable tuple for caching
        # args format: (pubkey_hash, desired_share_target, desired_pseudoshare_target, merged_addresses)
        cache_key = args[:-1] + (tuple(sorted(args[-1].items())) if args[-1] else (),)
        
        if cache_key not in self._cache:
            x, handler = self._inner.get_work(*args)
            self._cache[cache_key] = x, handler, 0
        
        x, handler, nonce = self._cache.pop(cache_key)
        ...
        if nonce + 1 != 2**self._my_bits:
            self._cache[cache_key] = x, handler, nonce + 1
