...
class Node(object):
    ...
    def start(self):
        ...
        def handle_header(new_header):
            ...
            if not (self.net.PARENT.POW_FUNC(bitcoin_data.block_header_type.pack(new_header)) <= self.coind_work.value['bits'].target):
                return
            ...
            if (self.best_block_header.value is None
                or (
                    ...
                    self.net.PARENT.BLOCKHASH_FUNC(bitcoin_data.block_header_type.pack(self.best_block_header.value)) == coind_best_block
                ) # new is child of current and previous is current
                or (
                    self.net.PARENT.BLOCKHASH_FUNC(bitcoin_data.block_header_type.pack(new_header)) == coind_best_block and
