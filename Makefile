...
${CACHE_ROOT}/virtualenv/virtualenv-1.11.6.tar.gz:
	mkdir -p ${CACHE_ROOT}/virtualenv
	sh -c "cd ${CACHE_ROOT}/virtualenv && curl -O https://pypi.python.org/packages/source/v/virtualenv/virtualenv-1.11.6.tar.gz"

${PKG_ROOT}/.stamp-h: conf/requirements*.pip ${CACHE_ROOT}/virtualenv/virtualenv-1.11.6.tar.gz
	...
	tar \
	  ...
	  -xf "${CACHE_ROOT}"/virtualenv/virtualenv-1.11.6.tar.gz
	python "${CACHE_ROOT}"/virtualenv/virtualenv-1.11.6/virtualenv.py \
	  --clear \
	  --distribute \
	  --never-download \
	  --prompt="(p2pool) " \
	  "${PKG_ROOT}"
	-rm -rf "${CACHE_ROOT}"/virtualenv/virtualenv-1.11.6
